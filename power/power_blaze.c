...
static void blaze_power_init(struct power_module *module)
{
    struct blaze_power_module *blaze =
                                   (struct blaze_power_module *) module;
    int tmp;
...
static int boostpulse_open(struct blaze_power_module *blaze)
{
    char buf[80];

    pthread_mutex_lock(&blaze->lock);

    if (blaze->boostpulse_fd < 0) {
        blaze->boostpulse_fd = open(BOOSTPULSE_PATH, O_WRONLY);

        if (blaze->boostpulse_fd < 0) {
            if (!blaze->boostpulse_warned) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGE("Error opening %s: %s\n", BOOSTPULSE_PATH, buf);
                blaze->boostpulse_warned = 1;
            }
        }
    }

    pthread_mutex_unlock(&blaze->lock);
    return blaze->boostpulse_fd;
}

static void blaze_power_set_interactive(struct power_module *module,
                                               int on)
{
    struct blaze_power_module *blaze =
                                   (struct blaze_power_module *) module;
...
static void blaze_power_hint(struct power_module *module,
                                    power_hint_t hint, void *data)
{
    ...
    case POWER_HINT_INTERACTION:
        if (boostpulse_open(blaze) >= 0) {
            len = write(blaze->boostpulse_fd, "1", 1);
...
struct blaze_power_module HAL_MODULE_INFO_SYM = {
    .base = {
        .common = {
            ...
            .name = "Blaze Power HAL",
            .author = "The Android Open Source Project",
            .methods = &power_module_methods,
        },

       .init = blaze_power_init,
       .setInteractive = blaze_power_set_interactive,
       .powerHint = blaze_power_hint,
    },
